              ...
              <div key={item.id} className='flex flex-row justify-between items-center w-full my-5 '>
                <div className='flex justify-between w-full items-center'>
                  <div className='flex items-center'>
                    <Link className='w-[40px] h-[40px] max-md:w-[40px] max-md:h-[40px] max-md:mr-2 rounded-full' onClick={() => setOpen(false)} to={`/product/details?id=${item.id}`}>
                    ...
                    <span className='font-semibold ml-2 text-xs max-md:text-xs'>({item.color.name} - {item.size})</span>
                  </div>
                  <div className='flex items-center justify-center'>
                    <span className='font-bold max-md:mx-1 text-green-600 mr-2'>R${item.price}</span>
                    <button onClick={() => removeFromCart(item.id,)} className='hover:scale-110'>
                      <FaTrash />
                    </button>
                  </div>
                </div>
